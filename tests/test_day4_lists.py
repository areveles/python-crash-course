...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../exercises/day4')))

import lists  # import your functions

from unittest.mock import patch

def test_sum_of_numbers():
    assert lists.sum_of_numbers([1, 2, 3, 4, 5]) == 15
    assert lists.sum_of_numbers([-1, 1]) == 0

def test_second_largest():
    assert lists.second_largest([1, 2, 3, 4, 5]) == 4
    assert lists.second_largest([10, 10, 5, 7]) == 7
    assert lists.second_largest([1]) == "Not enough unique numbers"

def test_reverse_list():
    assert lists.reverse_list([1, 2, 3]) == [3, 2, 1]
    assert lists.reverse_list([]) == []
    assert lists.reverse_list([5]) == [5]
